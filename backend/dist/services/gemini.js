...
class GeminiService {
     ...
     */
    static async generateCampaignRecommendations(analytics, optimization) {
...
${JSON.stringify(payload, null, 2)}`;
     ...
     */
    static async answerAnalyticsQuestion(analytics, question) {
        const model = this.getModel();
...
- **Actions**:
  ${payload.reallocations.map((r) => `- **${r.name} (${r.platform})**: Adjust budget by **${r.changePercent}** (New Target: **${r.recommendedSpend}**).`).join('\n  ')}
...
`;
    ...
    static getMockChatResponse(question, context) {

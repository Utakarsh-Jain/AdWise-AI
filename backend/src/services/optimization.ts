import { AnalyticsService, CampaignSummary } from './analytics';
...
export interface BudgetReallocation {
  ...
  efficiency: number;            // Conversions per Dollar (Conversions / Spend)
  recommendedSpend: number;      // New suggested budget
  changeAmount: number;          // recommended - current
  changePercentage: number;      // (changeAmount / current) * 100
  expectedConversions: number;   // recommendedSpend * efficiency
}
...
export class OptimizationService {
   ...
   */
  public static async optimizeBudget(userId: string): Promise<OptimizationResult> {

...
export async function getAiRecommendations(req: AuthRequest, res: Response) {
  try {
    ...
    const optimization = await OptimizationService.optimizeBudget(userId);
    ...
    // 2. Query Gemini API
    const recommendations = await GeminiService.generateCampaignRecommendations(analytics, optimization);
...
export async function postChat(req: AuthRequest, res: Response) {
  try {
    ...
    // 2. Query Gemini
    const reply = await GeminiService.answerAnalyticsQuestion(analytics, message);
